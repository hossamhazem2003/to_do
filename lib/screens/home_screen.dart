...
import 'package:to_do/firebase_auth.dart';
...
class _MyAppHomeState extends State<MyAppHome> {
  ...
  void addDataToFirestore() {
    try {
      getUsersCollection().doc(UserDm.currentUser!.id).collection('tasks').doc().set({
        'title': titleController.text,
        'description': descriptionController.text,
        'selectedTime': provider.selectedTime.millisecondsSinceEpoch,
        'isDone': false,
        'id' : getUsersCollection().doc(UserDm.currentUser!.id).collection('tasks').doc().id //000000000000000000000000000
      });
                              ...
                              child: Container(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    IconButton(
                                      onPressed: () async {
                                        await getUsersCollection().doc(UserDm.currentUser!.id).collection('tasks').doc(provider.todos[index].id)
                                            .delete();
                                        provider.getDataFromFirestore();
                                      },
